...
userRoute.post('/signup', async (c) => {
	...
	const validate=signupInp.safeParse(body);
	if(!validate.success){
	...
	const secret=env<Bindings>(c); //access of .toml file given to c
...
context varible c,is given access to external .toml file via "const secret=env<Bindings>(c)";
## Tyescript begins to complain if dont explicitly define the types of Bindings bcz hono doesn't have the the permission
to the .toml directly,it doesn't know what gonna come environment variable here .toml file https://hono.dev/helpers/adapter#env https://github.com/honojs/hono/issues/799
